...
import { Petition, Comment, WardInfo } from "./src/types";
...
export async function createApp() {
  ...
  function mapDbRowToWard(raw: any): WardInfo {
    return {
      wardNo: Number(raw.ward_no ?? raw.wardNo),
      name: raw.name || "",
      keyStreets: Array.isArray(raw.key_streets) ? raw.key_streets : (Array.isArray(raw.keyStreets) ? raw.keyStreets : []),
      inchargeName: raw.incharge_name || raw.inchargeName || "",
      inchargePhone: raw.incharge_phone || raw.inchargePhone || "",
      totalIssues: Number(raw.total_issues ?? raw.totalIssues ?? 0),
      resolvedIssues: Number(raw.resolved_issues ?? raw.resolvedIssues ?? 0),
    };
  }

  async function fetchWardsFromDb(): Promise<WardInfo[]> {
    if (!supabase) return [...wards];
    const { data, error } = await supabase.from('wards').select('*').order('ward_no', { ascending: true });
    if (error) throw error;
    return (data || []).map(mapDbRowToWard);
  }

  async function insertWardToDb(ward: WardInfo): Promise<WardInfo> {
    if (!supabase) {
      wards.push(ward);
    ...
    const { data, error } = await supabase.from('wards').insert([{
      ward_no: ward.wardNo,
      name: ward.name,
      key_streets: ward.keyStreets,
      incharge_name: ward.inchargeName,
      incharge_phone: ward.inchargePhone,
      total_issues: ward.totalIssues,
      resolved_issues: ward.resolvedIssues,
    }]).select('*').single();
    if (error) throw error;
    return mapDbRowToWard(data);
  }

  async function updateWardInDb(currentWardNo: number, ward: WardInfo): Promise<WardInfo | null> {
    if (!supabase) {
      const existing = wards.find(item => item.wardNo === currentWardNo);
      if (!existing) return null;
      Object.assign(existing, ward);
      wards.sort((a, b) => a.wardNo - b.wardNo);
      return existing;
    }
    const { data, error } = await supabase.from('wards').update({
      ward_no: ward.wardNo,
      name: ward.name,
      key_streets: ward.keyStreets,
      incharge_name: ward.inchargeName,
      incharge_phone: ward.inchargePhone,
    }).eq('ward_no', currentWardNo).select('*').single();
    if (error && error.code === 'PGRST116') return null;
    if (error) throw error;
    return mapDbRowToWard(data);
  }

  async function deleteWardFromDb(wardNo: number): Promise<boolean> {
    if (!supabase) {
    ...
    const { data, error } = await supabase.from('wards').delete().eq('ward_no', wardNo).select('ward_no');
    if (error) throw error;
  ...
  app.get("/api/wards", async (req, res) => {
    try {
      res.json(await fetchWardsFromDb());
    } catch (error) {
  ...
  app.post("/api/admin/wards", async (req, res) => {
    ...
    try {
      res.status(201).json(await insertWardToDb(ward));
    } catch (error) {
      console.error("Error creating ward:", error);
      res.status(500).json({ error: "Failed to create ward. Configure the wards table in Supabase." });
    }
  });

  app.put("/api/admin/wards/:wardNo", async (req, res) => {
    if (!requireAdmin(req, res)) return;
    ...
    try {
      const updated = await updateWardInDb(currentWardNo, {
        ...ward,
        wardNo: nextWardNo,
        name: String(name).trim(),
        keyStreets: streets,
  ...
  app.delete("/api/admin/wards/:wardNo", async (req, res) => {
    if (!requireAdmin(req, res)) return;

    const wardNo = Number(req.params.wardNo);
    try {
      const deleted = await deleteWardFromDb(wardNo);
      if (!deleted) return res.status(404).json({ error: "Ward not found" });
