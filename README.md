...
This app can expose a public frontend on GitHub Pages, but the backend API must still run on a separate Node host because GitHub Pages does not support the Express API server.

Recommended setup:
1. Deploy the backend to Render, Railway, Fly.io, or a similar Node host.
2. Set the frontend API base URL to that backend URL in the GitHub Pages build environment.
3. Publish the static frontend from the GitHub repository using the included Pages workflow.

The frontend is configured to use repo-relative asset paths when it is deployed to GitHub Pages. Set `VITE_API_BASE_URL` to your hosted backend URL before building.
...
Use a Node-capable host such as Render, Railway, Fly.io, or Vercel configured for a custom server. GitHub Pages is suitable only for the static frontend, not the backend API.
